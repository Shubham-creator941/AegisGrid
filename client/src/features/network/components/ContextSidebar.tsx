...
export function ContextSidebar() {
  ...
  const riskData = [
    { name: 'Very High', value: 20, color: 'var(--color-status-critical)' },
    { name: 'High', value: 30, color: 'var(--color-status-warning)' },
    { name: 'Moderate', value: 30, color: 'var(--color-status-normal)' },
    { name: 'Low', value: 20, color: 'var(--color-status-neutral)' },
  ];
  ...
  return (
    <div className="w-full h-full flex flex-col gap-4">
      ...
      <div className="bg-[#0B1120] border border-[#1E293B] rounded-[var(--radius-lg)] p-5 flex flex-col shadow-sm">
        <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-3 border-b border-[#1E293B] pb-2">Top Disruption Risk</h3>
        
        <div className="flex justify-between items-start mb-2">
          <div className="font-semibold text-sm text-slate-200">Strait of Hormuz Blockade</div>
          <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border text-status-critical bg-status-critical/10 border-status-critical/30">CRITICAL</span>
        </div>
        ...
        <div className="flex justify-between items-end mt-2">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              <span className="text-2xl font-light text-status-critical tracking-tight">72</span>
              <span className="text-xs text-slate-500 font-medium">/100</span>
            </div>
          </div>
          <div className="text-xs text-status-critical font-medium flex flex-col items-end">
            ...
            <svg viewBox="0 0 50 15" className="w-12 h-4 mt-1 overflow-visible">
              <polyline points="0,15 10,12 20,15 30,8 40,10 50,2" fill="none" stroke="var(--color-status-critical)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
        </div>
      </div>
      ...
      {/* Market Snapshot */}
      <div className="bg-[#0B1120] border border-[#1E293B] rounded-[var(--radius-lg)] p-5 flex flex-col shadow-sm">
        <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-3 border-b border-[#1E293B] pb-2">Market Snapshot (USD)</h3>
          ...
          {marketData.map(item => (
            <div key={item.name} className="flex justify-between items-center text-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`flex items-center text-xs font-semibold w-12 justify-end ${item.isUp ? 'text-status-normal' : 'text-status-critical'}`}>
      ...
      {/* Network Status */}
      <div className="bg-[#0B1120] border border-[#1E293B] rounded-[var(--radius-lg)] p-5 flex flex-col shadow-sm">
        <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-3 border-b border-[#1E293B] pb-2">Network Status</h3>
        <div className="flex flex-col gap-2.5 text-sm">
          <div className="flex justify-between">
            <span className="text-slate-400">Geopolitical Risk</span>
            <span className="text-status-critical font-semibold">High</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Logistics Risk</span>
            <span className="text-status-warning font-semibold">Elevated</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Supply Risk</span>
            <span className="text-status-normal font-semibold">Moderate</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400">Market Risk</span>
            <span className="text-status-normal font-semibold">Normal</span>
          </div>
        </div>
      </div>

      {/* Latest Intelligence */}
      <div className="bg-[#0B1120] border border-[#1E293B] rounded-[var(--radius-lg)] p-5 flex flex-col flex-1 shadow-sm overflow-hidden">
        <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-3 border-b border-[#1E293B] pb-2 flex items-center gap-2">
          <ShieldAlert size={14} className="text-status-recommended" />
          Latest Intelligence
        </h3>
          ...
          {intelligenceFeed.map((intel, idx) => (
            <div key={idx} className="flex gap-3">
              ...
              <div className="flex flex-col">
                ...
                <div className="text-[10px] text-status-recommended opacity-80 uppercase tracking-widest">Source: {intel.source}</div>
              </div>
            </div>
          ))}
        </div>
